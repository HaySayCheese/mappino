...
(function() {
  ...
  MPublicationService = (function() {
    ...
    MPublicationService.prototype.sendMessageToSeller = function(id_part, customer, successCallback, errorCallback) {
      var request;
      request = this.http.post("/ajax/api/notifications/send-message/" + id_part + "/", customer);
      request.success(function(response) {
        if (response.code === 0) {
          _.isFunction(successCallback) && successCallback(response);
        }
  ...
  angular.module('mappino.pages.map').factory('MPublicationService', [
    '$http', function(http) {
