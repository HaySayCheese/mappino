...
class ViewedPublicationsForCustomer(models.Model):
    ...
    class Meta:
        db_table = 'ViewedPublicationsForCustomer'
    ...
    def add(cls, customer_id, tid, hash_id):
        ...
        """
        try:
            record = ViewedPublicationsForCustomer.objects.filter(customer_id=customer_id).only('publications_ids')[:1][0]
        except IndexError:
            record = ViewedPublicationsForCustomer.objects.create(customer=customer_id)
    ...
    def remove(cls, customer_id, tid, hash_id):
        """
        Removes publication from customer's viewed.
        ...
        try:
            ...
            raise ViewedPublicationsForCustomer('No customer with such id.')
...
class ViewedPublicationsForPublication(models.Model):
    ...
    class Meta:
        db_table = 'ViewedPublicationsForPublication'
    ...
    def add(cls,customer_id,publication_ids):
        ...
        '''
        try:
            record = ViewedPublicationsForPublication.objects.filter(publication_ids=publication_ids).\
                only('customers_ids')[:1][0]
        except IndexError:
            record = ViewedPublicationsForPublication.objects.create(publication_ids=publication_ids)
    ...
    @classmethod
    def remove(cls,publication_ids):
        try:
            record = ViewedPublicationsForPublication.objects.filter(publication_ids=publication_ids).\
                only('customers_ids')[:1][0]
            record.delete()
