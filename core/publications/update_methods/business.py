...
from core.publications.models import BusinessesHeads, BusinessesRentTerms, BusinessesBodies
...
def update_business(prefix, value, head_id=None, body_id=None, rent_id=None):
	try:
		# bool
		if prefix == 'for_sale':
			if value == 'true':
				h = BusinessesHeads.by_id(head_id, head_id='for_sale')
				h.for_sale = True
		...
		# bool
			...
			if value == 'true':
				h = BusinessesHeads.by_id(head_id, head_id='for_rent')
				h.for_rent = True
				h.save(force_update=True)
		...
		# sid
			...
			b = BusinessesHeads.by_id(body_id, head_id='red_line')
			b.red_line = value
		...
		# blank or decimal
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='price')
				b.price = None
				b.save(force_update=True)
		...
		# sid
			...
			b = BusinessesBodies.by_id(body_id, only='transaction_type_sid')
		...
		# sid
			...
			b = BusinessesBodies.by_id(body_id, only='currency_sid')
		...
		# bool
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='price_is_contract')
		...
		# text
			...
			b = BusinessesBodies.by_id(body_id, only='add_terms')
			if not value:
				b.add_terms = None
				b.save(force_update=True)
		...
		# sid
			...
			r = BusinessesRentTerms.by_id(rent_id, only='period_sid')
			r.period_sid = value
		...
		# blank or decimal
			...
			if not value:
				r = BusinessesRentTerms.by_id(rent_id, only='price')
				r.price = None
				r.save(force_update=True)
		...
		# sid
			...
			r = BusinessesRentTerms.by_id(rent_id, only='currency_sid')
		...
		# boolean
			...
			if value == 'true':
				r = BusinessesRentTerms.by_id(rent_id, only='price_is_contract')
		...
		# text
			...
			r = BusinessesRentTerms.by_id(rent_id, only='add_terms')
			if not value:
				r.add_terms = None
				r.save(force_update=True)
		...
		# text
			...
			h = BusinessesHeads.by_id(head_id, head_id='title')
			if not value:
				h.title = None
				h.save(force_update=True)
		...
		# text
			...
			h = BusinessesHeads.by_id(head_id, head_id='descr')
			if not value:
				h.descr = None
				h.save(force_update=True)
				return
		...
		# blank or float
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='age')
				b.age = None
				b.save(force_update=True)
		...
		# blank or int
		elif prefix == 'workers_count':
			if not value:
				b = BusinessesBodies.by_id(body_id, only='workers_count')
		...
		# blank or decimal
		elif prefix == 'monthly_costs':
			if not value:
				b = BusinessesBodies.by_id(body_id, only='monthly_costs')
		...
		# sid
		elif prefix == 'mc_currency':
			value = int(value)
			if value not in CURRENCIES.values():
				raise ValueError

			b = BusinessesBodies.by_id(body_id, only='mc_currency_sid')
		...
		# blank or decimal
		elif prefix == 'annual_receipts':
			if not value:
				b = BusinessesBodies.by_id(body_id, only='annual_receipts')
		...
		# sid
		elif prefix == 'ar_currency':
			value = int(value)
			if value not in CURRENCIES.values():
				raise ValueError

			b = BusinessesBodies.by_id(body_id, only='ar_currency_sid')
		...
		# blank or decimal
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='share')
				b.share = None
				b.save(force_update=True)
		...
		# sid
			...
			b = BusinessesBodies.by_id(body_id, only='building_type_sid')
		...
		# blank or int
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='build_year')
				b.build_year = None
				b.save(force_update=True)
		...
		# sid
			...
			b = BusinessesBodies.by_id(body_id, only='condition_sid')
		...
		# blank or int
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='floor')
				b.floor = None
				b.save(force_update=True)
		...
		# sid
			...
			b = BusinessesBodies.by_id(body_id, only='floor_type_sid')
		...
		# blank or int
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='floors_count')
				b.floors_count = None
				b.save(force_update=True)
				return
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='mansard')
				b.mansard = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='ground')
				b.ground = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='lower_floor')
				b.lower_floor = True
				b.save(force_update=True)
		...
		# blank or float
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='halls_area')
				b.halls_area = None
				b.save(force_update=True)
		...
		# blank or float
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='total_area')
				b.total_area = None
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='closed_area')
				b.closed_area = True
				b.save(force_update=True)
		...
		# blank or float
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='plot_area')
				b.plot_area = None
				b.save(force_update=True)
		...
		# blank or int
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='vcs_count')
				b.vcs_count = None
				b.save(force_update=True)
		...
		# blank or float
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='ceiling_height')
		...
		# sid
			...
			b = BusinessesBodies.by_id(body_id, only='heating_type_sid')
		...
		# text
			...
			b = BusinessesBodies.by_id(body_id, only='custom_heating_type')
			if not value:
		...
		# sid
			...
			b = BusinessesBodies.by_id(body_id, only='ind_heating_type_sid')
		...
		# text
			...
			b = BusinessesBodies.by_id(body_id, only='custom_ind_heating_type')
			if not value:
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='electricity')
				b.electricity = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='gas')
				b.gas = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='security_alarm')
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='fire_alarm')
				b.fire_alarm = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='hot_water')
				b.hot_water = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='cold_water')
				b.cold_water = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='canalisation')
				b.canalisation = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='ventilation')
				b.ventilation = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='security')
				b.security = True
				b.save(force_update=True)
		...
		# text
			...
			b = BusinessesBodies.by_id(body_id, only='add_facilities')
			if not value:
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='phone')
				b.phone = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='mobile_coverage')
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='internet')
				b.internet = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='cable_tv')
				b.cable_tv = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='lan')
				b.lan = True
				b.save(force_update=True)
		...
		# blank or int
			...
			if not value:
				b = BusinessesBodies.by_id(body_id, only='phone_lines_count')
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='parking')
				b.parking = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='open_air')
				b.open_air = True
				b.save(force_update=True)
			...
			b = BusinessesBodies.by_id(body_id, only='add_buildings')
			if not value:
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='transport_stop')
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='bank')
				b.bank = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='market')
				b.market = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='cash_machine')
				b.cash_machine = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='cafe')
				b.cafe = True
				b.save(force_update=True)
		...
		# boolean
			...
			if value == 'true':
				b = BusinessesBodies.by_id(body_id, only='entertainment')
		...
		# text
			...
			b = BusinessesBodies.by_id(body_id, only='add_showplaces')
			if not value:
