...
def update_house(prefix, value, head_id=None, body_id=None, rent_id=None):
	try:
		...
		if prefix == 'for_sale':
			if value == 'true':
				h = HousesHeads.by_id(head_id, only='for_sale')
				h.for_sale = True
		...
		# bool
			...
			if value == 'true':
				h = HousesHeads.by_id(head_id, only='for_rent')
				h.for_rent = True
				h.save(force_update=True)
		...
		# text
			...
			h = HousesHeads.by_id(head_id, only='title')
			if not value:
				h.title = None
				h.save(force_update=True)
		...
		# text
			...
			h = HousesHeads.by_id(head_id, only='descr')
			if not value:
				h.descr = None
				h.save(force_update=True)
				return
