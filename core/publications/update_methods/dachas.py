...
from core.publications.constants import INDIVIDUAL_HEATING_TYPES, HEATING_TYPES, OBJECT_CONDITIONS, MARKET_TYPES, LIVING_RENT_PERIODS, SALE_TRANSACTION_TYPES
from core.publications.models import DachasBodies, DachasRentTerms, DachasSaleTerms
...
def update_dacha(h, field, value, tid):
	try:
		# bool
		if field == 'for_sale':
		...
		# boolean
		elif field == 'irr_water':
			if (value is True) or (value is False):
				b = DachasBodies.objects.filter(id=h.body_id).only('id')[0]
				b.irrigation_water = value
				b.save(force_update=True)
				return
			else:
				raise ValueError()
