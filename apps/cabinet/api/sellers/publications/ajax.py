                    ...
                    'data': [
                        {
                            ...
                            'date_enter': datetime.datetime.fromordinal(reservation.date_enter.toordinal()).replace(
                                tzinfo=tz).strftime('%Y-%m-%dT%H:%M:%S%z'),
                            'date_leave': datetime.datetime.fromordinal(reservation.date_leave.toordinal()).replace(
                                tzinfo=tz).strftime('%Y-%m-%dT%H:%M:%S%z'),
    ...
    def __briefs_of_section(cls, section, user_id):
        ...
        for tid in OBJECTS_TYPES.values():
            ...
            if section == 'all':
                ...
                query = query.filter(state_sid=OBJECT_STATES.unpublished(), deleted=None).order_by('state_sid',
                                                                                                   'created')
            elif section == 'trash':
